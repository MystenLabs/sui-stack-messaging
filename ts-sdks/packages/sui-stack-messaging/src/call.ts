...
import type { SuiGroupsCall } from '@mysten/sui-groups';

import type { EnvelopeEncryption } from './encryption/envelope-encryption.js';
import * as messaging from './contracts/sui_stack_messaging/messaging.js';
...
export interface SuiStackMessagingCallOptions {
 ...
 * ```
 */
export class SuiStackMessagingCall {
	#packageConfig: SuiStackMessagingPackageConfig;
	// eslint-disable-next-line @typescript-eslint/no-explicit-any -- Call only uses context-independent methods (generateGroupDEK, generateRotationDEK)
	#encryption: EnvelopeEncryption<any>;
	#derive: SuiStackMessagingDerive;
	...
	constructor(options: SuiStackMessagingCallOptions) {
	 ...
	 *
	 * @throws {SuiStackMessagingClientError} if SuiNS config was not provided
	 */
