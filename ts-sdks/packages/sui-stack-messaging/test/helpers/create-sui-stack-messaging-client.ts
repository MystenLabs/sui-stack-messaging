...
import {
	createSuiStackMessagingClient as createClient,
	type RelayerConfig,
...
/**
 * Creates a fully extended Sui client with `suiGroups`, `seal`,
 * and `suiStackMessaging` extensions.
 *
...
export function createSuiStackMessagingClient<TApproveContext = void>(
	...
	const baseClient = createSuiClient({
		...
		mvr: {
			overrides: {
				packages: {
					'@local-pkg/sui-groups': permissionedGroupsPackageId,
					'@local-pkg/sui-stack-messaging': messagingPackageId,
				},
			},
		},
	});
...
/** Convenience type for the return value of `createSuiStackMessagingClient` with default (void) approve context. */
export type SuiStackMessagingTestClient = ReturnType<typeof createSuiStackMessagingClient<void>>;
