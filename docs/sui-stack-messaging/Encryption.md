...
Seal operations require a **session key**, a short-lived key that authorizes decryption requests to Seal key servers. The SDK manages session keys internally through three tiers. See [Setup](./Setup.md) for configuration details.
